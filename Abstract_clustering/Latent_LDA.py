...
# Load your DataFrame (adjust filename and sheet as needed)
df = pd.read_excel("Abstracts.xlsx", sheet_name="Talks")
...
unwanted_words = [
    "previous", "highly", "identified", "potential", "high", "increased", "using", "the", "and", 
    "of", "work", "panel", "little", "including", "used", "analysed", "study", "studies", 
    "remains", "key", "understanding", "new", "talk", "presence", "level", "insight", 
    "populations", "genes", "insight"
]
...
def preprocess_text(text):
    ...
    # Perform POS tagging to filter verbs, adjectives, and adverbs
...
# --- Add LDA for Topic Modeling ---
n_topics = 6  # Define the number of topics
lda = LatentDirichletAllocation(n_components=n_topics, random_state=42)
lda_topic_distributions = lda.fit_transform(X)  # Get document-topic distributions

# Perform K-means clustering on LDA topic distributions
kmeans_lda = KMeans(n_clusters=6, random_state=42)
kmeans_lda_labels = kmeans_lda.fit_predict(lda_topic_distributions)

# Perform PCA for LDA clustering visualization
pca_lda = PCA(n_components=2)
pca_lda_result = pca_lda.fit_transform(lda_topic_distributions)
...
fig, ax = plt.subplots(figsize=(8, 6))
scatter = ax.scatter(pca_result[:, 0], pca_result[:, 1], c=agg_labels, cmap='rainbow', edgecolors='k')
plt.legend(*scatter.legend_elements(), title="Clusters")
...
# Plot PCA with LDA-based K-means clusters
fig, ax = plt.subplots(figsize=(8, 6))
scatter = ax.scatter(pca_lda_result[:, 0], pca_lda_result[:, 1], c=kmeans_lda_labels, cmap='rainbow', edgecolors='k')
plt.legend(*scatter.legend_elements(), title="Clusters")
plt.title("PCA Plot of LDA-based K-means Clusters")
plt.xlabel('PCA Component 1')
plt.ylabel('PCA Component 2')
save_plot(fig, "PCA_LDA_KMeans_Clusters.png")
...
# Save Agglomerative Clustering clusters
save_clustering_results(agg_labels, "agg", vectorizer, abstracts, X)

# Save LDA-based K-means clusters
save_clustering_results(kmeans_lda_labels, "lda_kmeans", vectorizer, abstracts, lda_topic_distributions)
